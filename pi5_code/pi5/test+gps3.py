...
# ==================================================================
# GPS CONFIG - RELAXED FOR YOUR GPS
# ==================================================================
GPS_PORT = "/dev/ttyAMA0"
GPS_BAUD = 9600
MIN_SATS = 3  # Changed from 4 to 3 (your GPS gets 3 satellites)
MAX_HDOP = 10.0  # Changed from 3.0 to 10.0 (your GPS has HDOP ~7.7)
GPS_DEBUG = True  # Enable debug output
...
class GPSManager:
    """Manages GPS reading with Kalman filtering in a separate thread."""
    def __init__(self, port=GPS_PORT, baud=GPS_BAUD):
        ...
        # Current best fix with default values
        self.current_fix = {
            "lat": None,
            "lon": None,
            "alt": None,
    ...
    def _is_port_available(self):
        ...
        try:
            test_ser = serial.Serial(self.port, self.baud, timeout=0.1, exclusive=True)
            test_ser.close()
            return True
    ...
    def _connect_gps(self):
        """Connect to GPS with proper port handling."""
        try:
            if self.ser:
                self.ser.close()
                self.ser = None
            ...
            if not self._is_port_available():
                print(f"Warning: {self.port} is in use by another process")
            ...
            print(f"Could not open GPS port {self.port}: {e}")
            self.port_in_use = True
            return False
        except Exception as e:
            print(f"Unexpected error opening GPS: {e}")
            return False
    
    def _run(self):
        """Main GPS reading loop running in separate thread."""
            ...
            try:
                ...
                if not raw:
                    ...
                    if consecutive_errors > 20:
                        print("GPS: No data received, reconnecting...")
                        self.ser = None
                        consecutive_errors = 0
                    continue
                ...
                # Debug: Show raw NMEA sentences occasionally
                if GPS_DEBUG and debug_counter % 50 == 0:
                    print(f"GPS RAW: {line[:60]}...")
                debug_counter += 1
                ...
                if GPS_DEBUG and debug_counter % 10 == 0:
                    print(f"GPS GGA: fix={fix}, sats={sats}, hdop={hdop:.1f}, lat={lat:.6f}, lon={lon:.6f}")
                
                # Validate fix with relaxed criteria for your GPS
                if fix == 0 or lat == 0.0:
                    self.skip_count += 1
                    if GPS_DEBUG and self.skip_count % 20 == 0:
                        print(f"GPS: Skipped (fix={fix}, lat={lat})")
                    continue
                
                if sats < MIN_SATS:
                    self.skip_count += 1
                    if GPS_DEBUG and self.skip_count % 20 == 0:
                        print(f"GPS: Skipped (sats={sats} < {MIN_SATS})")
                    continue
                
                if hdop > MAX_HDOP:
                    self.skip_count += 1
                    if GPS_DEBUG and self.skip_count % 20 == 0:
                        print(f"GPS: Skipped (hdop={hdop:.1f} > {MAX_HDOP})")
                    continue
    ...
    def get_best_fix(self):
        """Get the current best GPS fix (thread-safe)."""
        with self.lock:
            # If fix is old (>5 seconds), mark as invalid
            if self.current_fix["valid"] and (time.time() - self.last_fix_time) > 5:
                fix = self.current_fix.copy()
                fix["valid"] = False
                return fix
            return self.current_fix.copy()
    ...
    def get_status_string(self):
        ...
        fix = self.get_best_fix()
        if fix["valid"]:
            return f"GPS: {fix['lat']:.6f}, {fix['lon']:.6f} | sats={fix['sats']} | hdop={fix['hdop']:.1f}"
        return "GPS: No fix"
    
    def is_working(self):
        """Check if GPS is getting fixes."""
        fix = self.get_best_fix()
        return fix["valid"] and self.fix_count > 0
...
def save_frame_pair(pair_dir, frame_idx, rgb_frame, thermal_frame, elapsed, 
                    panel_results, thermal_label, thermal_skipped, gps_fix):
    """Save the current frame pair to disk with GPS data."""
    try:
        ...
        # Create combined display with GPS overlay
        ...
        cv2.putText(combined, info_text, (10, combined.shape[0] - 70),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        # Add GPS info if available
        if gps_fix and gps_fix["valid"]:
            gps_text = f"GPS: {gps_fix['lat']:.6f}, {gps_fix['lon']:.6f} | alt={gps_fix['alt']:.1f}m | sats={gps_fix['sats']} | hdop={gps_fix['hdop']:.1f}"
        else:
            gps_text = "GPS: No fix"
        cv2.putText(combined, gps_text, (10, combined.shape[0] - 45),
        ...
        with open(save_dir / "metadata.txt", "w") as f:
            ...
            f.write("\n=== GPS DATA ===\n")
            if gps_fix and gps_fix["valid"]:
                f.write(f"Latitude: {gps_fix['lat']:.7f}\n")
                f.write(f"Longitude: {gps_fix['lon']:.7f}\n")
                f.write(f"Altitude: {gps_fix['alt']:.2f}m\n")
                f.write(f"Raw Latitude: {gps_fix['lat_raw']:.7f}\n")
                f.write(f"Raw Longitude: {gps_fix['lon_raw']:.7f}\n")
                f.write(f"HDOP: {gps_fix['hdop']:.2f}\n")
                f.write(f"Satellites: {gps_fix['sats']}\n")
                f.write(f"Fix Type: {gps_fix['fix_type']} ({'DGPS' if gps_fix['fix_type']==2 else 'GPS'})\n")
                f.write(f"GPS Timestamp: {gps_fix['timestamp']}\n")
                f.write(f"Fix Count: {gps_fix['fix_count']}\n")
            else:
                f.write("No valid GPS fix available\n")
            
            # Results
            if thermal_skipped:
                f.write(f"\nThermal: SKIPPED (no panels detected)\n")
            else:
                f.write(f"\nThermal Label: {thermal_label}\n")
        ...
        # Save GPS data as JSON for easy parsing
        if gps_fix and gps_fix["valid"]:
            with open(save_dir / "gps.json", "w") as f:
                json.dump(gps_fix, f, indent=2)
...
def main():
    ...
    parser.add_argument("--disable-gps", action="store_true",
                         help="Disable GPS logging (useful for testing without GPS)")
    parser.add_argument("--gps-debug", action="store_true",
                         help="Enable GPS debug output")
    args = parser.parse_args()

    # Set global debug flag
    global GPS_DEBUG
    GPS_DEBUG = args.gps_debug
    ...
    print(f"GPS Criteria: MIN_SATS={MIN_SATS}, MAX_HDOP={MAX_HDOP}")
    print(f"GPS Debug: {'ON' if GPS_DEBUG else 'OFF'}")

    # Start GPS Manager (unless disabled)
    gps_manager = None
    if not args.disable_gps:
        try:
            gps_manager = GPSManager()
            # Give GPS time to get first fix
            time.sleep(2)
        except Exception as e:
    ...
    # Open CSV - extend with GPS columns
    with open(csv_path, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            "thermal_skipped",
            "gps_lat", "gps_lon", "gps_alt", "gps_sats", "gps_hdop", "gps_fix_valid"
        ])
        ...
        print("\n" + "="*120)
        print(f"{'Frame':>6} {'Elapsed':>8} {'RGB(ms)':>8} {'Panels':>6} {'Anom':>5} {'Thermal':>10} {'Conf':>6} {'FPS':>6} {'Saved':>6} {'GPS Sats':>8} {'HDOP':>6} {'GPS Lat':>12}")
        print("="*120)
        ...
        try:
            while running and (time.time() - start_time) < args.duration:
                ...
                # Get current GPS fix (if available)
                gps_fix = None
                if gps_manager:
                    gps_fix = gps_manager.get_best_fix()
                ...
                if args.save_frames:
                    ...
                    if should_save:
                        if save_frame_pair(pairs_dir, frame_idx, rgb_frame, thermal_frame, 
                                         elapsed, rgb_result["panel_results"], th_label, 
                                         thermal_skipped, gps_fix):
                            saved_count += 1
                            saved_this_frame = True
                
                # Get GPS status for display
                gps_sats = gps_fix["sats"] if gps_fix and gps_fix["valid"] else 0
                gps_hdop = gps_fix["hdop"] if gps_fix and gps_fix["valid"] else 0
                gps_valid = gps_fix["valid"] if gps_fix else False
                gps_lat = gps_fix["lat"] if gps_fix and gps_fix["valid"] else 0
                ...
                # GPS display
                if gps_valid:
                    gps_display = f"{gps_sats:8d} {gps_hdop:6.1f} {gps_lat:12.6f}"
                else:
                    gps_display = "   NO FIX            "
                
                print(f"{frame_idx:6d} {elapsed:8.2f} {rgb_result['total_ms']:8.2f} "
                      f"{n_panels:6d} {n_anomalies:5d} "
                      f"{thermal_display} {th_conf:6.3f} {fps:6.1f} {saved_indicator:>6} {gps_display}")
                
                # Write to CSV with GPS data
                writer.writerow([
                    ...
                    f"{fps:.2f}",
                    thermal_skipped,
                    f"{gps_fix['lat']:.7f}" if gps_fix and gps_fix["valid"] else "",
                    f"{gps_fix['lon']:.7f}" if gps_fix and gps_fix["valid"] else "",
                    f"{gps_fix['alt']:.2f}" if gps_fix and gps_fix["valid"] else "",
                    gps_sats,
                    f"{gps_hdop:.2f}" if gps_valid else "",
                    gps_valid
                ])
                f.flush()
            ...
            if gps_manager:
                print(f"   GPS fixes: {gps_manager.fix_count}, skipped: {gps_manager.skip_count}")
