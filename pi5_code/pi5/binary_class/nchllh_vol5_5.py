...
# CLAHE object created once as module-level singleton
...
def apply_clahe_fast(img: np.ndarray, clahe_obj) -> np.ndarray:
    """Optimized CLAHE - reuses existing CLAHE object"""
...
class Pipeline:
    def __init__(self):
        ...
        # Use module-level singleton — no new object needed
        self.clahe_obj = _CLAHE_OBJ
    ...
    def clahe_worker(self):
        # Use module-level singleton directly — already warmed up in warmup()
        local_clahe = _CLAHE_OBJ
        ...
        while not self._stop.is_set():
            ...
            fd.clahe = apply_clahe_fast(fd.raw, local_clahe)
    ...
    def detect_worker(self):
        while not self._stop.is_set():
            ...
            if crops:
                ...
                # Run in sub-batches of 8 to keep memory and latency bounded
                MAX_BATCH = 8
                all_logits = []
                for b_start in range(0, len(batch), MAX_BATCH):
                    sub = batch[b_start:b_start + MAX_BATCH]
                    all_logits.append(
                        self.classifier.run(None, {self.cls_input_name: sub})[0]
                    )
                logits_batch = np.concatenate(all_logits, axis=0)
