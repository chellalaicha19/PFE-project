...
FRAME_SKIP           = 2          # Consider removing if it causes instability
QUEUE_SIZE           = 4
...
class Pipeline:
    ...
    def detect_worker(self):
        while not self._stop.is_set():
            ...
            t0 = time.perf_counter()
            # Always run inference (removed frame-skip caching to improve stability)
    ...
    def run(self, image_paths: List[str]):
        ...
        # Run determinism check (optional but recommended)
        if image_paths:
        ...
        self.q_save.put(None)   # sentinel to stop save_worker
        self.q_save.join() 
        self._stop.set()
