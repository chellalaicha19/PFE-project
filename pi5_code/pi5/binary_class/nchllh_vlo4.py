...
class Pipeline:
    def __init__(self):
        ...
        # NOTE: no q_detect — classify runs inline in detect_worker
    ...
    def detect_worker(self):
        """YOLO detection + classification inline — never run simultaneously."""
        while not self._stop.is_set():
            try:
            ...
            # --- Classify inline (no thread hop, no CPU contention with YOLO) ---
            ...
            self.q_done.put(fd)
            # NOTE: classify_worker has been removed — no separate thread needed
    ...
    def run(self, image_paths: List[str]):
        ...
        threads = [
            ...
            # classify_worker REMOVED — runs inline inside detect_worker
