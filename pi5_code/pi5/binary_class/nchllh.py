"""
Full parallel pipeline: CLAHE → YOLO+Classify (merged) → Save
Workers: clahe_worker | detect_worker (YOLO + classify inline) | save_worker
"""
...
UNCERTAIN_BAND       = 0.10       # 50–60% = Uncertain

# Pre-allocated normalization constants
_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32).reshape(3, 1, 1)
_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32).reshape(3, 1, 1)

# Create CLAHE object ONCE globally
...
def preprocess_crop(crop: np.ndarray, size: int = 224) -> np.ndarray:
    """Optimized: single-pass BGR→RGB, CHW, normalize with pre-allocated constants."""
    crop = cv2.resize(crop, (size, size))
    x = np.ascontiguousarray(crop[:, :, ::-1], dtype=np.float32)  # BGR→RGB
...
class Pipeline:
    def __init__(self):
        ...
        sess_options.intra_op_num_threads = 2          # safe on Pi5 in threaded context
        ...
        # Two queues only: raw → clahe → done
        # detect_worker handles YOLO + classify inline (no contention)
    ...
    def warmup(self):
        ...
        # Realistic non-zero image to exercise all CLAHE tile paths
        dummy_real = np.random.randint(100, 200, (640, 640, 3), dtype=np.uint8)
        apply_clahe(dummy_real)
        apply_clahe(dummy_real)   # twice to stabilize tile cache
    ...
    def clahe_worker(self):
        """Stage 1: apply CLAHE, push to q_clahe."""
    ...
    def detect_worker(self):
        """Stage 2: YOLO detection + classify inline.
        Merged into one worker so YOLO and classifier never compete for CPU.
        """
        while not self._stop.is_set():
            try:
            ...
            # --- Classify (inline, no extra thread hop) ---
            ...
            if crops:
                batch = np.concatenate(
                    [preprocess_crop(c, CLASSIFIER_IMGSZ) for c in crops], axis=0
                )
    ...
    def save_worker(self):
        """Stage 3 (background): annotate and write result images.
        Non-blocking — never delays the collect loop.
        """
        while True:
    ...
    def run(self, image_paths: List[str]):
        ...
        threads = [
            ...
            # NOTE: NO classify_worker — classification runs inside detect_worker
        ]
        for t in threads:
            t.start()
        ...
        while collected < fed:
            ...
            self.q_save.put(fd)    # non-blocking hand-off to save_worker

        # Graceful shutdown
        self.q_save.put(None)      # stop save_worker
        self.q_save.join()         # wait for all writes to finish
        self._stop.set()           # stop clahe + detect workers
