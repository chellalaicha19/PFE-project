...
With stability fixes applied (inline classification + better warmup).
"""
...
YOLO_CONF            = 0.35
YOLO_IOU             = 0.45
YOLO_IMGSZ           = 416          # ← Must stay 640 for NCNN export
...
class Pipeline:
    def __init__(self):
        ...
        sess_options.intra_op_num_threads = 2   # ← Critical: 4 crashes on Pi 5
        ...
        self.q_done   = queue.Queue()          # q_detect removed
    ...
    def warmup(self):
        ...
        # Warm up CLAHE with realistic content (zeros don't exercise tiles properly)
        print("Warming up CLAHE...")
        dummy_real = np.random.randint(100, 200, (640, 640, 3), dtype=np.uint8)
        apply_clahe(dummy_real)
        apply_clahe(dummy_real)   # twice for tile cache stabilization
    ...
    def detect_worker(self):
        """Now also does classification inline (removes queue jitter)."""
        while not self._stop.is_set():
            try:
    ...
    def run(self, image_paths: List[str]):
        ...
        threads = [
            ...
            # classify_worker removed — now inline in detect_worker
        ]
        for t in threads:
            t.start()
