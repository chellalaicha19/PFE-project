...
OUTPUT_FOLDER        = "/home/pi5/panel_detection2/pipeline_results_v11"
...
class Pipeline:
    ...
    def save_worker(self):
        ...
        # Mark task done for None sentinel if needed
        try:
            self.q_save.task_done()
        except:
            pass
    ...
    def run(self, image_paths: List[str]):
        ...
        # Start save worker as daemon so it doesn't block exit
        save_thread = threading.Thread(target=self.save_worker, daemon=True, name="save")
        save_thread.start()
        
        clahe_thread = threading.Thread(target=self.clahe_worker, daemon=True, name="clahe")
        detect_thread = threading.Thread(target=self.detect_worker, daemon=True, name="detect")
        ...
        # Clear any remaining items in queues to prevent blocking
