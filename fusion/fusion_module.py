...
class PVInspectionPipeline:
    ...
    TEST_FOLDER        = "/Users/mac/Documents/PFE/test_rgb+thermal"
    ...
    def process_panel(self, crop_rgb: np.ndarray, img_therm: np.ndarray, panel_id: int) -> FusionResult:
        ...
        # Multiclass stage
        rgb_probs = self.predict_rgb_multi(crop_rgb)
        therm_probs = self.predict_thermal_multi(img_therm)

        print(f"    Panel {panel_id} - RGB   probs : { {c: f'{p:.3f}' for c, p in zip(RGB_MULTI_CLASSES, rgb_probs)} }")
        print(f"    Panel {panel_id} - Therm probs : { {c: f'{p:.3f}' for c, p in zip(THERMAL_MULTI_CLASSES, therm_probs)} }")

        return self._fuse(rgb_probs, therm_probs, stage="full", panel_id=panel_id)
    ...
    def _fuse(self, rgb_probs: np.ndarray, thermal_probs: np.ndarray, stage: str = "full", panel_id: int = 1) -> FusionResult:
        rgb_idx = int(np.argmax(rgb_probs))
        thermal_idx = int(np.argmax(thermal_probs))
        rgb_conf = float(rgb_probs[rgb_idx])
        thermal_conf = float(thermal_probs[thermal_idx])
